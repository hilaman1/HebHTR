...
//		const size_t maxT = buf.shape[0]; // max_seq_len =32
//		const size_t maxB = buf.shape[1]; // batch size
//		const size_t maxC = buf.shape[2]; // num_of_chars = 69
//
//		// check tensor size
...
//};
    ...
    std::vector<std::vector<uint32_t>> compute(const py::array_t<double, py::array::c_style | py::array::forcecast>& array) const
    {
        ...
        std::cout << "before_batch_res" << std::endl;

        // Create a single vector to store the concatenated results
        std::vector<std::vector<uint32_t>> concatenatedResults;
        ...
        for (size_t b = 0; b < maxB; ++b)
        {
            ...
            // Apply decoding algorithm to batch element
            auto batchResult = wordBeamSearch(mat, m_beamWidth, m_lm, m_lmType);
             std::cout << "after_batch_res" << std::endl;

            // Append the result for this batch element to the concatenatedResults
            concatenatedResults.insert(concatenatedResults.end(), batchResult.begin(), batchResult.end());
        }
